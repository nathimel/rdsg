...
class RothErevReinforcementLearning(Dynamics):
    ...
    def run(self):
        ...
        for _ in tqdm(range(self.num_rounds)):
            ...
            # track data
            self.game.data["points"].append(agents_to_point(
                speaker=sender,
                listener=receiver,
...
class DiscreteTimeReplicatorDynamic(Dynamics):
    ...
    def run(self):
        ...
        while not converged:
            ...
            U_sender = np.array([
                [R[w] @ U[s] for w in range(n_signals)] 
            ...
            if (np.abs(S - S_prev).sum() < self.threshold and np.abs(R - R_prev).sum() < self.threshold) or (i == self.max_its):
                converged = True
